...
-- ---- labeling queue: the WHOLE labeling job lives here (no JSON index).
--      populate_labeler.py seeds rows (status 'pending') with the image url and
--      candidate species; the labeler pulls ONE pending row at a time, the user
--      resolves it, and the row flips to 'done'. Shared effort: once an item is
--      'done' it's never served again, so any user can label without collisions.
--      pull_conflicts.py reads the 'done' rows back into the dataset. ----
create table if not exists label_items (
  stem        text primary key,          -- image id (matches dataset filename)
  url         text not null,             -- public Storage url of the image
  candidates  jsonb not null default '[]'::jsonb,   -- proposed species (conflicts)
  status      text not null default 'pending',      -- 'pending' | 'done'
  label       jsonb,                     -- result: {"species":[...], "noise":bool}
  labeled_by  uuid references auth.users(id),
  updated_at  timestamptz not null default now()
);
alter table label_items enable row level security;
create policy "li_auth_read"   on label_items for select
  using (auth.role() = 'authenticated');
create policy "li_auth_insert" on label_items for insert
  with check (auth.role() = 'authenticated');
create policy "li_auth_update" on label_items for update
  using (auth.role() = 'authenticated');
create index if not exists label_items_status_idx on label_items (status);